...
export class MemoryStorage implements IStorage {
  ...
  async upsertUser(userData: any): Promise<User> {
    // For Replit auth - find existing user by email or create new one
    ...
    if (existingUser) {
      ...
      const newUser: User = {
        ...
        password: 'replit-auth', // Placeholder for Replit auth users
        firstName: userData.firstName || null,
        lastName: userData.lastName || null,
  ...
  async getAppointments(userId: number, userType: 'doctor' | 'patient'): Promise<Appointment[]> {
    if (userType === 'doctor') {
      return this.appointments.filter(apt => apt.doctorId === userId);
    } else {
      return this.appointments.filter(apt => apt.patientId === userId);
    }
  }
